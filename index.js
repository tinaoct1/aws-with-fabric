	...
	try {
		...
		const getImageUrl = () => {
			...
			if (!imageUrl) {
				...
				const text = parsedBody.userLanguage === 'ar' ? 'الرجاء ارفاق تغريدتكم بالصورة التي تودون إضافة الفلتر إليها. ;)' : 'Dear, please make sure you attach an image to your tweet in order to receive your personalized edit. ;)'
				...
				return fabric.Image.fromURL(imageUrl, (img) => {
					...
					if ((img.width < 100 || img.width > 2000) && (img.height < 100 || img.height > 2000)) {
						...
						const invalidImageText = parsedBody.userLanguage === 'ar' ? 'أبعاد الصورة المسموح بها هي XXXX الرجاء إعادة إرسال الصورة بالأبعاد الصحيحة لاستلام صورتك الشخصية المعدلة مع الفلتر الخاص.'
						...
						return fabric.Image.fromURL('https://s3.amazonaws.com/api-project-81-staging/Option-4.png', (overlayImage) => {
							...
							pipeline.on('uploaded', details => {
								return httpResponse(null, {
									...
									bucketName + '/overlayed-images/' + parsedBody.userTwitterUid + '.png', text: "Here is image"
								}, callback)
							})
